...
const { assertRestorationPaths } = require('./validate-storage');
...
(async () => {
  ...
  const job = buildMachinistJob({ ids, purpose: 'restoration', ext });
  ...
  while (Date.now() < deadline) {
    try {
      await assertRestorationPaths(ids.tenant_id, ids.asset_id, ext);
      // Ensure ai_version is absent
